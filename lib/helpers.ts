...
export const checkIfCliProgramIsInstalled = (
  cmd: string
...
export const sortFileInfo = (
  fileInfo: IFileInfoC[]
): IFileInfoC[] => fileInfo.sort((a, b) => {
  if (a.hash > b.hash) {
    return -1
  }
  if (a.hash < b.hash) {
...
export const calcFileInfoContentHash = (
  fileInfo: IFileInfoC[]
) => {
  const sorted = sortFileInfo(fileInfo)
  const concatedHashes = sorted.reduce((acc, cur) => 
    acc += cur.hash
  , '')
...
`Repo ${
  ...
  ? ` on branch ${yellow(repoBranch)}`
  : ''
}${repoCommit
  ? ` from commit ${yellow(repoCommit)}`
  : ''
