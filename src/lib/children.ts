...
export const cloneGitRepository = (repo: string, fullOutputPath: string): Promise<string> =>
  runChildProcess(`git clone ${repo} ${fullOutputPath}`);

export const checkoutGitBranch = (workingFolder: string, repoBranch: string): Promise<string> =>
  runChildProcess(`cd ${workingFolder} && git checkout ${repoBranch}`);

export const checkoutGitCommit = (workingFolder: string, repoCommit: string): Promise<string> =>
  runChildProcess(`cd ${workingFolder} && git checkout ${repoCommit}`);

export const gitNewBranch = (workingFolder: string, newBranchName: string): Promise<string> =>
  runChildProcess(`cd ${workingFolder} && git checkout -b ${newBranchName}`);

export const gitCommit = (workingFolder: string, commitMessage: string): Promise<string> =>
  runChildProcess(`cd ${workingFolder} && git commit -m "${commitMessage}"`);

export const gitPushOrigin = (workingFolder: string, branchName: string): Promise<string> =>
...
export const buildDockerImage = (folder: string, dockerName: string): Promise<string> =>
...
export const normalizeEnumerateFiles = (basePath: string, files: IFileInfoA[]): IFileInfoB[] =>
  files.map(file => ({
    basePath,
    path: file.path,
...
export const addFileOrFolder = (files: IFileInfoC[]): Promise<IFileInfoD[]> =>
  Promise.all(
    files.map(async file => ({
      ...file,
      fileOrFolder: (await fse.lstat(file.path)).isDirectory() ? 'folder' : 'file'
    }))
  );
