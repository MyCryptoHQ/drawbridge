...
export function* pushFromAToB(
  ...
  logger.succeed(
    `Staged ${yellow(toDistFolder)} for commit on repo ${repoTo}`
  )
  ...
  logger.log(`Pushing repo ${yellow(repoTo)} to ${yellow('origin')}, watch for SSH password prompt`)
  yield call(
    gitPushOrigin,
    toWorkingFolder,
    newBranch
